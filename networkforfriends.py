"""
Network for friends is a project for first programming course at Tampere University
Program reads a file to get names and connections between those names. User is 
asked to tell a name and program prints all the connections for that name. User
can also add new names and connections. It is also possible to check if given two
names have mutual friends. 
Author: nmahonen
Note: There are lots of lines of codes we had from the beginning, only those blocks
of code with a comment starting with "nm:" are mine. E.g. all "DO NOT MODIFY..." are 
given and were not to be changed.
"""
...
def read_friendship_network(filename):
    ...
    """
    #nm: create a dict where are names as keys and list of friends as a payload
    network = {}
...
def add_friendship(network, name1, name2):
    ...
    #nm: to check the input to include two different names
    if name1 == name2:
        return

    #nm: add a list for name1
    if name1 not in network:
        network[name1] = []

    #nm: should name2 not be at name1 list, add it there
    if name2 not in network[name1]:
        network[name1].append(name2)

    #nm: add a list for name2
    if name2 not in network:
        network[name2] = []

    #nm: should name1 not be at name2 list, add it there
...
def print_command(network, namelist):
    ...
    #nm: loop through all names in alphabetical order and print names and 
    #connections also in alphabetical order (connections have '-' at the 
    #beginning of the line)
    for name in sorted(network):
        print(name)
        friends = sorted(network[name])
...
def common_friends_command(network, namelist):
    ...
    #nm: to create a set from name1 friends
    name1_friends = set()
    for name in network[name1]:
        name1_friends.add(name)

    #nm: to create a set from name2 friends
    name2_friends = set()
    for name in network[name2]:
        name2_friends.add(name)

    #nm: use intersection to check if the sets have common values (friends)
    #and if so, print these friends in alphabetical order
    common_friends = name1_friends & name2_friends
    for name in sorted(common_friends):
        print(name)

    #nm: should there be not common values, print info of it
